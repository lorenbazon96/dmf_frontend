...
function addPdfHeader(doc, title, userName, companyName) {
  ...
  try { doc.setFont("Roboto", "bold"); } catch { /* fallback */ }
  doc.text(title, logoBase64 ? 48 : 14, 16);

  doc.setFontSize(8);
  doc.setTextColor(100);
  try { doc.setFont("Roboto", "normal"); } catch { /* fallback */ }
...
export async function exportWorkersPdf(workers, userName, companyName) {
  ...
  autoTable(doc, {
    startY,
    head: [["#", "Ime i prezime", "Kontakt", "CP", "CS", "D", "G", "B", "W", "A", "Ocjena"]],
    body: workers.map((w, i) => [
      i + 1, w.fullName, w.contact,
...
export async function exportClientsPdf(clients, userName, companyName) {
  ...
  autoTable(doc, {
    startY,
    head: [["#", "Tip", "Naziv", "Država", "Adresa", "Kontakt", "Email", "OIB"]],
    body: clients.map((c, i) => [
      i + 1,
      c.clientType === "person" ? "Osoba" : "Firma",
      c.clientName, c.country, c.adressa,
      c.contact || "", c.email || "", c.oib || "",
    ]),
    ...tableStyles,
  });
...
export async function exportSingleWorkerPdf(worker, ratings, operations, totalRating, projectsCompleted, userName, companyName) {
  await initPdf();
  const doc = createDoc("p");
  const startY = addPdfHeader(doc, "Radnik: " + (worker.fullName || ""), userName, companyName);
  ...
  autoTable(doc, {
    ...
    body: [
      ["Rezanje cijevi", ratings.pipeCutting, operations.pipeCutting ? "Da" : "Ne"],
      ["Rezanje lima", ratings.sheetCutting, operations.sheetCutting ? "Da" : "Ne"],
...
export async function exportSingleClientPdf(form, responsiblePersons, userName, companyName) {
  await initPdf();
  const doc = createDoc("p");
  const startY = addPdfHeader(doc, "Klijent: " + (form.clientName || ""), userName, companyName);
  const t1 = autoTable(doc, {
    startY,
    head: [["Polje", "Vrijednost"]],
    body: [
  ...
  if (form.clientType === "company" && responsiblePersons?.length) {
    const filled = responsiblePersons.filter(p => p.fullName || p.email || p.contact);
    if (filled.length) {
      autoTable(doc, {
        startY: t1.finalY + 8,
  ...
  doc.save(`klijent-${(form.clientName || "klijent").replace(/\s+/g, "_")}.pdf`);
}
...
export function printWorkersList(workers, userName, companyName) {
  let rows = workers.map((w, i) =>
    `<tr><td>${i + 1}</td><td>${w.fullName}</td><td>${w.contact}</td><td>${w.cp ?? 0}</td><td>${w.cs ?? 0}</td><td>${w.d ?? 0}</td><td>${w.g ?? 0}</td><td>${w.b ?? 0}</td><td>${w.w ?? 0}</td><td>${w.a ?? 0}</td><td>${w.rating ?? 0}</td></tr>`
  ).join("");
  printContent("Radnici", `<table><tr><th>#</th><th>Ime i prezime</th><th>Kontakt</th><th>CP</th><th>CS</th><th>D</th><th>G</th><th>B</th><th>W</th><th>A</th><th>Ocjena</th></tr>${rows}</table>`, userName, companyName);
}

export function printClientsList(clients, userName, companyName) {
  let rows = clients.map((c, i) =>
    `<tr><td>${i + 1}</td><td>${c.clientType === "person" ? "Osoba" : "Firma"}</td><td>${c.clientName}</td><td>${c.country}</td><td>${c.adressa}</td><td>${c.contact || ""}</td><td>${c.email || ""}</td></tr>`
  ).join("");
  printContent("Klijenti", `<table><tr><th>#</th><th>Tip</th><th>Naziv</th><th>Država</th><th>Adresa</th><th>Kontakt</th><th>Email</th></tr>${rows}</table>`, userName, companyName);
}

export function printSingleWorker(worker, ratings, operations, totalRating, projectsCompleted, userName, companyName) {
  let info = `<table>
    ...
    <tr><td>Rezanje cijevi</td><td>${ratings.pipeCutting}</td><td>${operations.pipeCutting ? "Da" : "Ne"}</td></tr>
    <tr><td>Rezanje lima</td><td>${ratings.sheetCutting}</td><td>${operations.sheetCutting ? "Da" : "Ne"}</td></tr>
    <tr><td>Zavarivanje</td><td>${ratings.welding}</td><td>${operations.welding ? "Da" : "Ne"}</td></tr>
    <tr><td>Savijanje</td><td>${ratings.bending}</td><td>${operations.bending ? "Da" : "Ne"}</td></tr>
    <tr><td>Brušenje</td><td>${ratings.grinding}</td><td>${operations.grinding ? "Da" : "Ne"}</td></tr>
    <tr><td>Bušenje</td><td>${ratings.drilling}</td><td>${operations.drilling ? "Da" : "Ne"}</td></tr>
    <tr><td>Montaža</td><td>${ratings.assembly}</td><td>${operations.assembly ? "Da" : "Ne"}</td></tr>
  </table>`;
  printContent("Radnik: " + worker.fullName, info, userName, companyName);
}

export function printSingleClient(form, responsiblePersons, userName, companyName) {
  let info = `<table>
    <tr><th>Polje</th><th>Vrijednost</th></tr>
    <tr><td><b>Tip</b></td><td>${form.clientType === "person" ? "Osoba" : "Firma"}</td></tr>
  ...
  if (form.clientType === "company" && responsiblePersons?.length) {
    ...
    if (filled.length) {
      ...
      filled.forEach(p => { info += `<tr><td>${p.fullName}</td><td>${p.email}</td><td>${p.contact}</td><td>${p.note}</td></tr>`; });
      info += "</table>";
    }
  }
...
export async function exportWarehousePdf(items, userName, companyName) {
  await initPdf();
  const doc = createDoc("l");
  const startY = addPdfHeader(doc, "Skladište / Warehouse", userName, companyName);
  autoTable(doc, {
    startY,
...
export function printWarehouseList(items, userName, companyName) {
  let rows = items.map((item, i) =>
    `<tr><td>${i + 1}</td><td>${item.type || ""}</td><td>${item.name || ""}</td><td>${item.specs || ""}</td><td>${item.qty ?? 0}</td></tr>`
  ).join("");
  printContent("Skladište", `<table><tr><th>#</th><th>Tip</th><th>Naziv</th><th>Specifikacija</th><th>Količina</th></tr>${rows}</table>`, userName, companyName);
}

export async function exportHistoryPdf(projects, userName, companyName) {
  await initPdf();
  const doc = createDoc("l");
  const startY = addPdfHeader(doc, "Povijest proizvodnje / Production History", userName, companyName);
  autoTable(doc, {
    startY,
...
export function printHistoryList(projects, userName, companyName) {
  let rows = projects.map((p, i) =>
    `<tr><td>${i + 1}</td><td>${p.rn || ""}</td><td>${p.client || ""}</td><td>${p.name || ""}</td><td>${p.completedOn || ""}</td></tr>`
  ).join("");
  printContent("Povijest proizvodnje", `<table><tr><th>#</th><th>RN</th><th>Klijent</th><th>Naziv</th><th>Završeno</th></tr>${rows}</table>`, userName, companyName);
}

export async function exportAnalyticsPdf(stats, workerPerformance, userName, companyName) {
  await initPdf();
  const doc = createDoc("p");
  const startY = addPdfHeader(doc, "Analitika / Analytics", userName, companyName);
  const t1 = autoTable(doc, {
    startY,
  ...
  if (workerPerformance && workerPerformance.length) {
    autoTable(doc, {
      startY: t1.finalY + 8,
      head: [["Radnik", "Vrijednost", "Zadaci", "Prosječno vrijeme", "Efikasnost"]],
...
export function printAnalytics(stats, workerPerformance, userName, companyName) {
  let info = `<table>
    ...
    <tr><td><b>Prosječno trajanje</b></td><td>${stats.avgDuration || "0h"}</td></tr>
  ...
  if (workerPerformance && workerPerformance.length) {
    ...
    workerPerformance.forEach(w => {
      info += `<tr><td>${w.name || ""}</td><td>${w.value ?? ""}</td><td>${w.tasks ?? ""}</td><td>${w.avgTime || ""}</td><td>${w.efficiency || ""}</td></tr>`;
    });
    info += "</table>";
  }
...
export async function exportDashboardPdf(projects, assemblies, userName, companyName) {
  await initPdf();
  const doc = createDoc("l");
  const startY = addPdfHeader(doc, "Aktivni projekti / Active Projects", userName, companyName);
  const t1 = autoTable(doc, {
    startY,
...
export function printDashboard(projects, assemblies, userName, companyName) {
  ...
  projects.forEach(p => {
    info += `<tr><td>${p.rn || ""}</td><td>${p.name || ""}</td><td>${p.client || ""}</td><td>${p.worksOn || ""}</td><td>${p.progress ?? 0}</td><td>${p.est || "-"}</td></tr>`;
  });
  info += "</table>";
  if (assemblies && assemblies.length) {
    ...
    assemblies.forEach(a => {
      info += `<tr><td>${a.rn || ""}</td><td>${a.operation || ""}</td><td>${a.worksOn || ""}</td><td>${a.est || "-"}</td><td>${a.progress ?? 0}</td></tr>`;
    });
    info += "</table>";
  }
...
export async function exportProjectDetailPdf(project, drawings, productionPlan, userName, companyName) {
  await initPdf();
  const doc = createDoc("l");
  const startY = addPdfHeader(doc, "Projekt: " + (project.rn || "") + " " + (project.name || ""), userName, companyName);
  const t1 = autoTable(doc, {
    startY,
    head: [["Polje", "Vrijednost"]],
    body: [
  ...
  if (drawings && drawings.length) {
    ...
    if (productionPlan && productionPlan.length) {
      autoTable(doc, {
        startY: t2.finalY + 8,
        head: [["Br. nacrta", "Operacija", "Radi na", "EST", "Napredak (%)"]],
...
export function printProjectDetail(project, drawings, productionPlan, userName, companyName) {
  let info = `<table>
    <tr><th>Polje</th><th>Vrijednost</th></tr>
    ...
    <tr><td><b>Napredak</b></td><td>${(project.progress ?? 0)}%</td></tr>
    <tr><td><b>EST</b></td><td>${project.est || "-"}</td></tr>
  </table>`;
  if (drawings && drawings.length) {
    ...
    drawings.forEach(d => {
      info += `<tr><td>${d.no || ""}</td><td>${d.partName || ""}</td><td>${d.assembly || ""}</td><td>${d.weight || ""}</td><td>${d.qty ?? ""}</td></tr>`;
    });
    info += "</table>";
  }
  if (productionPlan && productionPlan.length) {
    info += `<br><h3 style="color:#2b579a;">Plan proizvodnje</h3><table><tr><th>Br. nacrta</th><th>Operacija</th><th>Radi na</th><th>EST</th><th>Napredak (%)</th></tr>`;
    productionPlan.forEach(p => {
      info += `<tr><td>${p.drawingNo || ""}</td><td>${p.operation || ""}</td><td>${p.worksOn || ""}</td><td>${p.est || "-"}</td><td>${p.progress ?? 0}</td></tr>`;
    });
    info += "</table>";
  }
  printContent("Projekt: " + (project.rn || "") + " " + (project.name || ""), info, userName, companyName);
}
